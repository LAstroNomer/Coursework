...
if __name__ == '__main__':
    ...
    for sr in cat:
        ...
        if sp[0] == '#':
            ...
            cat_tbl.add_row(sp)
    #print(hdr)
    cx = cat_tbl['ALPHAMODEL_J2000']
    cy = cat_tbl['DELTAMODEL_J2000']
    ...
    p = g = 0
    for x, y, gf, re, e1, e2, n, tp, sw in zip(cx, cy, cgf, cref, ce1, ce2, cn, ctp, switch):
        if tp < sw:
            print("%9d %5.3f %5.3f %6.3f " % (p, x, y, gf), file=p_file)
            p += 1
        else:
            print("%9d %14.8f %14.8f %6.3f %4.1f %3.1f %3.2f %3.2f" % (g, x, y, gf, re, n, e1, e2), file=g_file)
            g += 1
